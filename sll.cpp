...
int main() {
    ...
    ob.assign({1,2,3,4,5});
    SLL<int>::const_iterator ii = ob.cbegin(); 
    ob.insert_after(ii,0);
    ob.print();
    SLL<int>::iterator i= ob.begin();
    SLL<int>::const_iterator ab = ob.cbegin();
    SLL<int>::const_iterator bc = ob.cbegin();
    ++ab;
    ++bc;
    ++bc;
    ++bc;
    ++bc;
    ++bc;
    i = ob.erase_after(ab,bc);
    ob.swap(obj);
    
    ob.print();
    ob.assign({1,1,1,1,3, 5, 7});
    obj.assign({2,1,2,2,2,2,3,3,2, 4, 6, 8,4,4,4,4,1, 10});
    SLL<int>::const_iterator it = ob.cbegin();
    ++it;
    ob.splice_after(it, obj);
    obj.merge(ob);
    obj.print();
    obj.unique();
    obj.print();
